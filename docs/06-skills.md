...
v4 的 `run()` 把「构造 messages」和「循环」写在同一个方法里，`messages[0]` 在进入循环前就定死了，循环内部没有留出任何可以替换系统消息的切入点。而 v7 恰恰需要在**循环中途**换掉 `messages[0]`。

所以这不是 v7 想多写代码，而是 v4 当初没有为"运行时改系统提示"预留扩展点。这也是一个真实的工程经验：

**一个类能不能被干净地扩展，取决于它当初有没有在正确的位置留出接缝。**

如果 v4 一开始就把「构造系统消息」抽成一个可覆盖的方法，v7 就只需要覆盖那一个方法。你可以把这当作一个练习：试着改造 v4，让 v7 不必重写整个 `run()`。
