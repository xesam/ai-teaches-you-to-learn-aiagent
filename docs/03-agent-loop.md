...
v3 的 `execute_single_call()` 函数只处理了**一次**函数调用。  
...
**Q: 一轮循环、一次模型请求、一次工具调用，是同一件事吗？**
A: 不是。本项目每进入一轮循环，就向模型发一次请求（`for` 循环里只有一处 `create`）；但只有当模型的回复里含工具请求时才执行工具。以第 1 节那个"今天是几月几号？今天是今年第几天？"为例，循环跑 3 轮、请求模型 3 次，却只调用了 2 次工具——因为最后一轮模型不再请求工具，而是直接作答并结束循环。所以轮数（= 请求次数）通常比工具调用次数多一次。哪怕模型第一次就直接作答、一个工具都没调，它仍然走完了 Agent 循环的结构。
